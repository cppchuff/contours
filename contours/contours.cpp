...
const int g_dReferWidth = 190;//mm 
...

{
	...
	g_srcImage = imread("作业4图像1.tif", IMREAD_COLOR);

	//灰度 降低计算量

	cvtColor(g_srcImage, g_grayImage, COLOR_BGR2GRAY);
	imshow("test",g_grayImage);
	...
	Canny(g_grayImage, g_grayImage, 100, 250);
	...
	for (unsigned i = 0; i < g_vContours.size(); i++) {
		...
		if (contourArea(g_vContours[i]) < 500)//面积太小 则忽略
